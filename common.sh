...
schema_setup() {
  if [ "${schema_type}" == "mongo" ]; then
    ...
    mongo --host mongodb-dev.learndevopseasy.online.online </app/schema/${component}.js &>>${log_file}
    status_check $?
    ...
    print_head "Load Schema"
    mysql -h mysql-dev.learndevopseasy.online.online -uroot -p${mysql_root_password} < /app/schema/shipping.sql &>>${log_file}
    status_check $?
  fi
}
